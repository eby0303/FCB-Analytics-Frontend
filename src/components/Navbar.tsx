...
const Navbar = () => {
  ...
  return (
    <header className="sticky top-0 z-50 backdrop-blur-md bg-fcb-dark/90 dark:bg-fcb-dark/90 border-b border-white/10 dark:border-white/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden md:flex items-center space-x-8">
            ...
            <button
              onClick={toggleTheme}
              className="p-2 rounded-full bg-white/10 dark:bg-white/10 text-gray-300 dark:text-gray-300 hover:text-white dark:hover:text-white transition-colors"
            >
              {theme === 'dark' ? '☀️' : '🌙'}
            </button>
          </nav>
      ...
      {isOpen && (
        <div className="md:hidden fixed inset-0 z-40 bg-fcb-dark dark:bg-fcb-dark">
          <div className="fixed inset-0 bg-black/95 dark:bg-black/95">
            <div className="pt-20 pb-4 px-4 space-y-1">
              {navLinks.map((link) => (
                <NavLink
                  ...
                  to={link.path}
                  className={({ isActive }) => 
                    `block py-4 text-center text-lg font-medium border-b border-white/10 dark:border-white/10 ${
                      isActive ? 'text-[#FFED02] dark:text-[#FFED02]' : 'text-gray-300 dark:text-gray-300'
                    }`
                  }
                >
                  {link.name}
